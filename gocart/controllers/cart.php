...
//if there is no page id provided redirect to the homepage.
    ...
    function category($id) {
        ...
        foreach ($base_url as $slug) {
            $slugs = new stdClass();
            $slugs_array[$slug] = (string)$slug; // $this->Category_model->get_category_human_name_by_slug($slug);
        }
    ...
    function product($id) {
        ...
        //get the product
        $product = $this->Product_model->get_product_by_code($id);
        $data['product'] = $product;

        if (!empty($product->categories)) {
            foreach ($product->categories as $product_category_item) {
                $product_category_id = $product_category_item->category_id;
            }
            $tree = $this->Category_model->get_category_tree($product_category_id);
            if ($tree) {
                foreach ($tree as $tree_item) {
                    $slugs = new stdClass();
                    $slugs_array[$tree_item->slug] = (string) $tree_item->name;
                    $base_url[] = $tree_item->slug;
                }
            }
        } else {
            $slugs = new stdClass();
            $slugs_array[""] = "Результаты поиска";
            $base_url[] = "";
        }
...
//		$data['base_url']			= $this->uri->segment_array();
//		foreach($data['base_url'] as $slug){
//                    $slugs = new stdClass();
//                    $slugs_array[$slug] = (string)$this->Category_model->get_category_human_name_by_slug($slug);
//                }
        $data['slugs_array'] = $slugs_array;
        ...
        $data['gift_cards_enabled'] = $this->gift_cards_enabled;
        $tao_parameters = unserialize($product->tao_parameters);
        if (!is_array($tao_parameters)) {
            $tao_parameters = false;
        }
        $data['tao_parameters'] = $tao_parameters;
