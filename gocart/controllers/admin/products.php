...
class Products extends Admin_Controller {
    ...
    function form($id = false, $duplicate = false) {
        ...
        if ($id) {
            ...
            $data['tao_parameters'] = "";
            $data['tao_images'] = "";
            $data['product_secret_code'] = $product->slug;
...
//make sure we haven't submitted the form yet before we pull in the images/related products from the database
    ...
    private function get_tao_products() {
        $this->db->dbprefix = "";
        //$this->db->limit(10000);
        $this->db->select('id, good_id, options');
        $this->db->where('category_id !=', 0);
        $res = $this->db->get('orangetao_goods_tmp');
        $this->db->dbprefix = "system_";
        $result = $res->result();
        $res->free_result();
        return $result;
    }

    private function get_tao_record($id) {
        $this->db->dbprefix = "";
        $this->db->where('id', $id);
        $res = $this->db->get('orangetao_goods_tmp');
        $this->db->dbprefix = "system_";
    ...
    private function get_system_products() {
    ...
    private function check_if_tao_id_processed($tao_id) {
        $this->db->limit(1);
        $this->db->where('tao_id', $tao_id);
        $res = $this->db->get('products');
        $result = $res->row();
        $res->free_result();
        return $result;
    ...
    public function import() {
        ...
        $tao_products = $this->get_tao_products();

        if ($tao_products) {
            foreach ($tao_products as $tao_id) {
                if (!$this->check_if_tao_id_processed($tao_id->id)) {
                    $tao_item = $this->get_tao_record($tao_id->id);
                    $tao_imgs = unserialize($tao_item->images);
                    $img = array();
                    if (is_array($tao_imgs)) {
                        foreach ($tao_imgs as $key => $image_item) {
                            $unique_id = uniqid() . time() . uniqid();
                            if ($key == 0) {
                                $img[$unique_id]['primary'] = true;
                            }
                            $img[$unique_id]['filename'] = $image_item['URL'];
                            $img[$unique_id]['alt'] = "";
                            $img[$unique_id]['caption'] = "";
                        }
                    }

                    $slug = sha1($tao_item->good_id);
                    $route['slug'] = $slug;
                    $route_id = $this->Routes_model->save($route);

                    $save['id'] = "";
                    $save['sku'] = $tao_item->good_id;
                    $save['name'] = $tao_item->title;
                    $save['seo_title'] = $tao_item->title;
                    $save['meta'] = "";
                    $save['description'] = "";
                    $save['excerpt'] = "";
                    $save['price'] = $tao_item->price_end;
                    $save['saleprice'] = "";
                    $save['weight'] = "";
                    $save['track_stock'] = "1";
                    $save['fixed_quantity'] = "";
                    ...
                    $save['tao_id'] = $tao_item->good_id;
                    $save['tao_url'] = $tao_item->url;
                    $save['seller_id'] = $tao_item->seller_id;
                    $save['tao_options'] = $tao_item->options;
                    $save['tao_parameters'] = $tao_item->params;
                    $save['tao_images'] = $tao_item->images;
                    $save['product_secret_code'] = $slug;
                    $product_id = $this->Product_model->save($save, $options = false, $categories = false);

                    $category_products['product_id'] = $product_id;
                    $category_products['category_id'] = $tao_item->category_id;
                    $product_category_id = $this->db->insert('category_products', $category_products);
                }
                //var_dump($product_category_id);die;
            }
        }
    }
    
    public function fix_product_routes() {
        $this->load->model(array('Option_model', 'Category_model', 'Digital_Product_model'));
        $this->load->helper('text');
        $this->load->model('Routes_model');

        $products = $this->get_system_products();
        foreach($products as $item){
            $new_route['slug'] = $item->slug;
            $new_route['route'] = 'cart/product/'.$item->slug;
            $route_id = $this->Routes_model->save($new_route);
    ...
    public function import_options() {
        ...
        $this->load->helper('text');
        $tao_products = $this->get_tao_products();
        foreach ($tao_products as $tao_item) {
            $update['tao_id'] = $tao_item->good_id;
            $this->db->where('id', $tao_item->id);
            $res = $this->db->update('system_products', $update);

            $options = unserialize($tao_item->options);

            if (is_array($options)) {
                foreach ($options as $key_param => $param) {

                    $this->db->set('product_id', $tao_item->id);
                    $this->db->set('sequence', 0);
                    $this->db->set('name', $key_param);
                    $this->db->set('type', "radiolist");
                    $this->db->set('required ', "0");
                    $this->db->insert('system_options');
                    $option_id = $this->db->insert_id();

                    foreach ($param as $param_option) {
                        $this->db->set('option_id ', $option_id);
                        $this->db->set('name', $param_option);
                        $this->db->set('value', 0);
                        $this->db->set('price', 0);
                        $this->db->set('weight ', 0);
                        $this->db->set('sequence', 0);
                        $this->db->set('limit', NULL);
                        $option_value_id = $this->db->insert('system_option_values');
                    }
                }
            }
        }
