...
class Categories extends Admin_Controller {
    ...
    private function get_tao_categories() {
        $this->db->dbprefix = "";
        $res = $this->db->get('orangetao_categories');
    ...
    function import() {
        ...
        $this->load->library('form_validation');
         $this->load->helper('text');
        $this->load->model('Routes_model');
        $tao_categories = $this->get_tao_categories();
        if ($tao_categories) {
            foreach ($tao_categories as $source) {
                $slug = $source->title;

                $this->category_id = $source->id;
                ...
                $data['id'] = $source->id;
                $data['name'] = $slug;
                $data['slug'] = $source->alias;
                $data['description'] = "";
                //citation
                $data['excerpt'] = "";
                $data['sequence'] = "";
                $data['parent_id'] = $source->id_parent;
                $data['image'] = "";
                $data['seo_title'] = $source->alias;
                $data['meta'] = $source->alias;

                //$slug = $this->Routes_model->validate_slug($slug);
                $route['slug'] = $source->alias;
                $route_id = $this->Routes_model->save($route);
                
                $data['route_id'] = intval($route_id);
                
                $category_id = $this->Category_model->save($data, $ignore_id = true);
